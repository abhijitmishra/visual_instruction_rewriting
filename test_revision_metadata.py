...
MODEL_ID = "anonymoususerrevision/ReVision-250M-256-16-metadata" # ! Need to upload the new model once fine-tuned
...
def main(args):
    ...
    test_dataset = RevisionRewriteDatasetWithMetadata(split="test", use_auth_token=use_auth_token, processor=processor)
    ...
    generated_texts = []
    # TODO check padding. Warning appears about decoder-only architecture and right-padding...
    ...
    test_df.to_csv("test_predicted_metadata.tsv", sep='\t', index=False)

    print("Predictions saved to test_predicted_metadata.tsv")

def parse_arguments():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description="Evaluate the fine-tuned ReVision model on a test set (metadata).")
